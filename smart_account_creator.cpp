...
class sac : public contract {
	public:
      ...
      copy(owner_pubkey.data.begin(), owner_pubkey.data.end(), owner_pubkey_char.begin());
      
			array<char,33> active_pubkey_char;
      copy(active_pubkey.data.begin(), active_pubkey.data.end(), active_pubkey_char.begin());

      const auto owner_auth = authority{1,{{{(uint8_t)abieos::key_type::k1, owner_pubkey_char}, 1}},{},{}};
      const auto active_auth = authority{1,{{{(uint8_t)abieos::key_type::k1, active_pubkey_char}, 1}},{},{}};
       ...
       const auto fee = asset(std::max((transfer.quantity.amount + 119)/200, 1000ll));
       eosio_assert(cpu+net+amount+fee <= transfer.quantity, "Not enough money");

			 const auto remaining_balance = transfer.quantity - cpu - net - amount - fee;

       // create account
			 INLINE_ACTION_SENDER(call::eosio, newaccount)( N(eosio), {{_self, N(active)}},
               {_self, account_to_create, owner_auth, active_auth});
       
       // buy ram
       INLINE_ACTION_SENDER(call::eosio, buyram)( N(eosio), {{_self,N(active)}},
               {_self, account_to_create, amount});
       
       // delegate and transfer cpu and net
       INLINE_ACTION_SENDER(call::eosio, delegatebw)( N(eosio), {{_self,N(active)}},
               {_self, account_to_create, net, cpu, 1});
       // fee
       INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {{_self,N(active)}},
               {_self, string_to_name("saccountfees"), fee, std::string("Account creation fee")});
